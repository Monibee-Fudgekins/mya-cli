...
async function ensureAuthenticated(): Promise<SessionData | null> {
...
function getMarketStatusMessage(): string {
  const now = new Date();
...
async function submitAnalysisRequest(userId: string, machineId: string, analysisType: string, data: any = {}): Promise<any> {
...
program
  ...
  .action(async () => {
    try {
      ...
      try {
        const analysisResult = await submitAnalysisRequest(
          session.userId,
          session.machineId,
          'cmt_analysis',
          {}
        ...
        if (analysisResult.success && analysisResult.result) {
          ...
          const result = analysisResult.result as any;
